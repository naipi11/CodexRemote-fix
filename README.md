...
1. Download either `CodexRemote-fix-2.5.11-setup.exe` (recommended installer) or `CodexRemote-fix-2.5.11-windows-x64.zip` from [Releases](https://github.com/naipi11/CodexRemote-fix/releases). Download and verify the matching `.sha256.txt` before continuing.
...
Every tagged release ships a Windows setup installer and a portable ZIP, each with its SHA-256 checksum, release manifest, and the shared payload manifest. The `.github/workflows/release.yml` workflow builds both from the tag automatically, so v2.5.11 includes:

- `CodexRemote-fix-2.5.11-setup.exe` and its checksum
- `CodexRemote-fix-2.5.11-windows-x64.zip` and its checksum
- `CodexRemote-fix-2.5.11-payload-manifest.json`
- `CodexRemote-fix-2.5.11-release-manifest.json` and `CodexRemote-fix-2.5.11-setup-release-manifest.json`
