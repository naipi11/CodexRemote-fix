...
> [!NOTE]
> **发布状态：** v2.5.19 是当前稳定版。安装包、以 EXE 启动的便携 ZIP、托盘操作、重启并修复流程和常驻远程连接均纳入发布验证门禁。
...
1. 从 [Releases](https://github.com/naipi11/CodexRemote-fix/releases) 下载 `CodexRemote-fix-2.5.19-setup.exe`（推荐安装包）或便携 ZIP `CodexRemote-fix-2.5.19-windows-x64.zip`；下载并核对对应的 `.sha256.txt`。
...
每个带 tag 的发布都会附带 Windows 安装包和便携 ZIP，各自带 SHA-256 校验文件、release manifest，并共享 payload manifest。`.github/workflows/release.yml` 会在 tag 上自动构建两者，因此 v2.5.19 提供：

- `CodexRemote-fix-2.5.19-setup.exe` 及其校验文件
- `CodexRemote-fix-2.5.19-windows-x64.zip` 及其校验文件
- `CodexRemote-fix-2.5.19-payload-manifest.json`
- `CodexRemote-fix-2.5.19-release-manifest.json` 和 `CodexRemote-fix-2.5.19-setup-release-manifest.json`
