...
internal sealed class CcodInstallRuntime : IDisposable
{
    ...
    [MethodImpl(MethodImplOptions.Synchronized)] internal object OpenDirectory(object parentToken,string leaf,bool create)
    {
        ...
        OpenResult r=OpenRelative(parent.Native,leaf,LIST_DIRECTORY|ADD_FILE|ADD_SUBDIRECTORY|READ_ATTRIBUTES|DELETE|SYNC,SHARE_READ|SHARE_WRITE,create?OPEN_IF:OPEN,DIRECTORY|BACKUP_INTENT);
    ...
    [MethodImpl(MethodImplOptions.Synchronized)] internal void RemoveOwnedTree(object parentToken,string leaf)
    {
        ...
        string originalLeaf=target.Leaf,originalPath=target.Path,tombstone=".__ccod_delete_"+Guid.NewGuid().ToString("N");bool quarantined=false,released=false;
        try{ReleaseDescendants(plan);released=true;int rec=Rename(target.Native,parent.Native,tombstone);if(rec!=0)throw new Win32Exception(rec);MoveDirectoryPin(target,tombstone,Path.Combine(parent.Path,tombstone));quarantined=true;released=false;ReopenKnownTree(target);plan.Clear();BuildPlan(target,plan);RevalidatePlan(target,plan);foreach(Pin p in plan){if(p.Directory)continue;ValidateCurrent(p);int ec=SetDelete(p.Native,true);if(ec!=0)throw new Win32Exception(ec);marked.Add(p);ValidateMarkedFile(p);}RevalidateMarkedNamespace(target,new HashSet<Pin>(marked));}
        catch{for(int i=marked.Count-1;i>=0;i--)SetDelete(marked[i].Native,false);if(!released){ReleaseDescendants(plan);released=true;}if(quarantined){int back=Rename(target.Native,parent.Native,originalLeaf);if(back!=0)throw new Win32Exception(back);MoveDirectoryPin(target,originalLeaf,originalPath);quarantined=false;}ReopenKnownTree(target);throw;}
        foreach(Pin p in marked)RemovePin(p);
        foreach(Pin p in plan){if(!p.Directory)continue;ValidateCurrent(p);int ec=SetDelete(p.Native,true);if(ec!=0)throw new Win32Exception(ec);RemovePin(p);}
        ValidateCurrent(target);int tec=SetDelete(target.Native,true);if(tec!=0)throw new Win32Exception(tec);RemovePin(target);
    }
    ...
    private static void ValidateMarkedFile(Pin p){FILE_INFO i=Info(p.Native);if(i.VolumeSerialNumber!=p.Volume||(((ulong)i.FileIndexHigh<<32)|i.FileIndexLow)!=p.Index||(i.FileAttributes&ATTR_REPARSE)!=0||IsDirectory(i)||!OnlyDefaultStream(p.Native)||i.NumberOfLinks!=0)throw new InvalidDataException("namespace changed");}
    private void RevalidateMarkedNamespace(Pin dir,HashSet<Pin> marked){ValidateCurrent(dir);foreach(string n in Enumerate(dir.Native)){Pin p;if(!names.TryGetValue(Key(dir,n),out p)||!p.Owned)throw new InvalidDataException("namespace changed");if(p.Directory)RevalidateMarkedNamespace(p,marked);else if(marked.Contains(p))ValidateMarkedFile(p);else throw new InvalidDataException("namespace changed");}}
    ...
    private void ReopenKnownTree(Pin dir){List<Pin> children=new List<Pin>();foreach(Pin p in pins.Values)if(Object.ReferenceEquals(p.Parent,dir)&&!p.Closed)children.Add(p);foreach(Pin p in children){if(p.Directory){OpenResult r=OpenRelative(dir.Native,p.Leaf,LIST_DIRECTORY|ADD_FILE|ADD_SUBDIRECTORY|READ_ATTRIBUTES|DELETE|SYNC,SHARE_READ|SHARE_WRITE,OPEN,DIRECTORY|BACKUP_INTENT);p.Handle=r.Handle;ValidateCurrent(p);ReopenKnownTree(p);}else{OpenResult r=OpenRelative(dir.Native,p.Leaf,READ|WRITE|DELETE|SYNC,SHARE_READ,OPEN,NON_DIRECTORY|WRITE_THROUGH);p.Stream=new FileStream(r.Handle,FileAccess.ReadWrite,65536,false);r.Handle=null;ValidateCurrent(p);}}}
...
function Commit-CcodInstallPinnedPromotion { param([Parameter(Mandatory)]$Transaction,[Parameter(Mandatory)]$ParentDirectory,[Parameter(Mandatory)]$TemporaryLeaf,[Parameter(Mandatory)][string]$DestinationLeaf) Assert-CcodLeaf $DestinationLeaf;$p=Get-CcodPinToken $Transaction $ParentDirectory Directory;$t=Get-CcodPinToken $Transaction $TemporaryLeaf File;Convert-CcodRuntimeError {Invoke-CcodRuntimeMethod $p.State.Runtime Promote @($p.Token,$t.Token,$DestinationLeaf)|Out-Null} 'CCOD_INSTALL_PROMOTION_FAILED';return $TemporaryLeaf }
