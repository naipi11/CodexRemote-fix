...
$script:CcodProductReadyPhases=@('Prepared','PackageVerified','RuntimeStaged','PreviousProtectionStopped','RuntimePromoted','PointerCommitted','StableShellCommitted','ProtectionReady','Ready')
...
function Assert-CcodInstallProductReadyRecord($Record){
    $valid=Test-CcodInstallOrderedProperties $Record $script:CcodProductReadyFields
    if(-not$valid){Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SCOPE' 'Complete strict Ready transaction identity is required' $Record}
    $valid=$valid-and$Record.schemaVersion-is[int]-and$Record.schemaVersion-eq1-and$Record.transactionId-is[string]-and$Record.transactionId-cmatch'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
    $valid=$valid-and$Record.newRuntimeId-is[string]-and$Record.newRuntimeId-cmatch'^[A-Za-z0-9][A-Za-z0-9._-]{0,95}$'-and(Test-CcodInstallUnsignedInteger $Record.newGeneration)-and[uint64]$Record.newGeneration-gt0
    $valid=$valid-and$Record.newManifestSha256-is[string]-and$Record.newManifestSha256-cmatch'^[0-9a-f]{64}$'-and$Record.sealedPackageSha256-is[string]-and$Record.sealedPackageSha256-cmatch'^[0-9a-f]{64}$'
    $oldNull=$null-eq$Record.oldRuntimeId;$valid=$valid-and($oldNull-eq($null-eq$Record.oldGeneration))-and($oldNull-eq($null-eq$Record.oldManifestSha256))
    if(-not$oldNull){$valid=$valid-and$Record.oldRuntimeId-is[string]-and$Record.oldRuntimeId-cmatch'^[A-Za-z0-9][A-Za-z0-9._-]{0,95}$'-and(Test-CcodInstallUnsignedInteger $Record.oldGeneration)-and[uint64]$Record.oldGeneration-gt0-and$Record.oldManifestSha256-is[string]-and$Record.oldManifestSha256-cmatch'^[0-9a-f]{64}$'}
    $valid=$valid-and($Record.ownedObjectNames-is[Array]);$owned=@($Record.ownedObjectNames);$seen=[Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal);$valid=$valid-and$owned.Count-le64;foreach($name in $owned){$valid=$valid-and$name-is[string]-and$name-cmatch'^[A-Za-z0-9][A-Za-z0-9._-]{0,159}$'-and$seen.Add([string]$name)}
    $valid=$valid-and$Record.phase-is[string]-and(@($script:CcodProductReadyPhases)+'Failed')-ccontains$Record.phase-and(($Record.phase-ceq'Failed'-and$Record.errorCode-is[string]-and$Record.errorCode-cmatch'^CCOD_[A-Z0-9_]{1,96}$')-or($Record.phase-cne'Failed'-and$null-eq$Record.errorCode))
    if(-not$valid){Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SCOPE' 'Complete strict Ready transaction identity is required' $Record};return $true
}
...
function Get-CcodInstallProductReadyAuthority([string]$InstallRoot,$ReadyTransaction){
    [void](Assert-CcodInstallProductReadyRecord $ReadyTransaction);if($ReadyTransaction.phase-cne'Ready'){Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SCOPE' 'Terminal Ready transaction is required' $ReadyTransaction}
    $root=[IO.Path]::GetFullPath($InstallRoot).TrimEnd('\');try{
        ...
        $store=Join-Path $root 'state\install-transactions';$records=@();foreach($file in @(Get-ChildItem -LiteralPath $store -Force -ErrorAction Stop|Sort-Object Name)){if($file.PSIsContainer-or$file.Name-cnotmatch'^\d{20}\.\d{2}\.[A-Za-z]+\.[0-9a-f-]{36}\.json$'){throw 'unknown transaction object'};$record=Read-CcodInstallProductStrictJson $file.FullName $script:CcodProductReadyFields;[void](Assert-CcodInstallProductReadyRecord $record);$phaseIndex=if($record.phase-ceq'Failed'){99}else{[Array]::IndexOf($script:CcodProductReadyPhases,[string]$record.phase)};$expectedLeaf='{0:D20}.{1:D2}.{2}.{3}.json'-f[uint64]$record.newGeneration,$phaseIndex,$record.phase,$record.transactionId;if($file.Name-cne$expectedLeaf){throw 'transaction leaf is noncanonical'};$records+=,$record}
        $heads=@();foreach($group in @($records|Group-Object transactionId)){$chain=@($group.Group|Sort-Object @{Expression={if($_.phase-ceq'Failed'){99}else{[Array]::IndexOf($script:CcodProductReadyPhases,[string]$_.phase)}}});if($chain.Count-eq0-or$chain[0].phase-cne'Prepared'){throw 'transaction chain start'};$identity=$chain[0].PSObject.Copy();$identity.phase='Ready';$identity.errorCode=$null;for($index=0;$index-lt$chain.Count;$index++){$current=$chain[$index];$comparison=$current.PSObject.Copy();$comparison.phase='Ready';$comparison.errorCode=$null;if(($comparison|ConvertTo-Json -Depth 8 -Compress)-cne($identity|ConvertTo-Json -Depth 8 -Compress)){throw 'transaction identity changed'};if($index-gt0-and$current.phase-cne'Failed'-and[Array]::IndexOf($script:CcodProductReadyPhases,[string]$current.phase)-ne([Array]::IndexOf($script:CcodProductReadyPhases,[string]$chain[$index-1].phase)+1)){throw 'transaction chain gap'}};$heads+=,$chain[-1]}
        $wanted=$ReadyTransaction|ConvertTo-Json -Depth 8 -Compress;$matches=@($heads|Where-Object{$_.phase-ceq'Ready'-and($_|ConvertTo-Json -Depth 8 -Compress)-ceq$wanted});if($matches.Count-ne1){throw 'unique full Ready record missing'}
        return [pscustomobject]@{ReadyCanonical=$wanted;PersistedReadyCanonical=($matches[0]|ConvertTo-Json -Depth 8 -Compress);RuntimeId=[string]$ReadyTransaction.newRuntimeId;Generation=[uint64]$ReadyTransaction.newGeneration;ManifestSha256=[string]$manifestSha;PackageSha256=[string]$ReadyTransaction.sealedPackageSha256}
...
function Open-CcodInstallProductRegistrationTransaction {
    ...
    Initialize-CcodInstallRuntime;if(-not[IO.Path]::IsPathRooted($InstallRoot)){Throw-CcodInstallFileError 'CCOD_INSTALL_ROOT_INVALID' 'Install root must be absolute' $InstallRoot};$root=[IO.Path]::GetFullPath($InstallRoot);$authority=Get-CcodInstallProductReadyAuthority $root $ReadyTransaction
    $arguments=[object[]]@($root,$null);$token=Convert-CcodInstallRuntimeError {Invoke-CcodRuntimeStatic OpenProduct $arguments} 'CCOD_INSTALL_PRODUCT_SCOPE';$runtime=$arguments[1];$transaction=New-CcodInstallCapability;$script:CcodTransactions.Add($transaction,[pscustomobject]@{Runtime=$runtime;InstallRoot=$root.TrimEnd('\');RuntimeId=$authority.RuntimeId;RootToken=$token;StateOnly=$false;ProductOnly=$true;ReadyAuthority=$authority;Closed=$false;CleanupFailed=$false;Retired=$false;RetirementResult=$null;Disposition=$null});$script:CcodScopes.Add($transaction,[pscustomobject]@{Transaction=$transaction;Token=$token;Kind='ProductTransaction';ReadOnly=$true;RuntimeId=$authority.RuntimeId});$transaction
}
