...
internal sealed class CcodInstallGenerationRuntimeV4 : IDisposable
{
    ...
    {
        if(disposed)throw new ObjectDisposedException("transaction");
    ...
    [MethodImpl(MethodImplOptions.Synchronized)] internal object CopyProductShortcut(object folderToken,string kind,string source,string leaf,long expectedLength,string expectedSha)
    {
        Pin parent=Require(folderToken,true);ValidateCurrent(parent);
        ...
        string full=Path.GetFullPath(source);SafeFileHandle sourceHandle=CreateFileW(full,READ|READ_ATTRIBUTES|SYNC,SHARE_READ,IntPtr.Zero,OPEN_EXISTING,FLAG_REPARSE,IntPtr.Zero);
        if(sourceHandle.IsInvalid)throw new Win32Exception(Marshal.GetLastWin32Error());FileStream sourceStream=null;
        try
        {
            FILE_INFO sourceInfo=Info(sourceHandle);ValidatePlain(sourceInfo,sourceHandle);if(!SamePath(FinalPath(sourceHandle),full))throw new InvalidDataException("source path changed");
            sourceStream=new FileStream(sourceHandle,FileAccess.Read,65536,false);sourceHandle=null;externalStreams.Add(sourceStream);FileStream ownedSource=sourceStream;sourceStream=null;
            if(ownedSource.Length!=expectedLength||Sha(ownedSource)!=expectedSha)throw new InvalidDataException("source mismatch");ownedSource.Position=0;
            Pin destination=CreateTemporaryFile(parent);ownedSource.CopyTo(destination.Stream);destination.Stream.Flush(true);
...
function Open-CcodInstallProductSpecialFolder {
    param([Parameter(Mandatory)]$FileTransaction,[Parameter(Mandatory)][ValidateSet('StartMenu','Desktop')][string]$Kind)
    $scope=Get-CcodInstallTransaction $FileTransaction 'CCOD_INSTALL_TRANSACTION_INVALID'
    if(-not[object]::ReferenceEquals($scope.Record.Transaction,$FileTransaction)){Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_FOLDER_INVALID' 'Product special folder requires the transaction-root capability' $Kind}
    ...
    Add-CcodInstallScope $FileTransaction $token ProductFolder $false $null
}
function Copy-CcodInstallProductShortcut {
    param([Parameter(Mandatory)]$Folder,[Parameter(Mandatory)][ValidateSet('StartMenu','Desktop')][string]$Kind,[Parameter(Mandatory)][string]$SourcePath,[Parameter(Mandatory)][string]$Leaf,[Parameter(Mandatory)][int64]$ExpectedLength,[Parameter(Mandatory)][string]$ExpectedSha256,[Parameter(Mandatory)]$FileTransaction)
    if($Leaf-cne'CodexRemote-fix.lnk'-or-not[IO.Path]::IsPathRooted($SourcePath)-or$ExpectedLength-lt 0-or$ExpectedSha256-cnotmatch'^[0-9a-f]{64}$'){Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SHORTCUT_INVALID' 'Invalid fixed product shortcut contract' $Leaf}
    $scope=Get-CcodInstallTransaction $Folder 'CCOD_INSTALL_PRODUCT_FOLDER_INVALID';if(-not[object]::ReferenceEquals($scope.Record.Transaction,$FileTransaction)-or$scope.Record.Kind-cne'ProductFolder'){Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_FOLDER_INVALID' 'Product folder capability is outside the file transaction' $Kind}
    Convert-CcodInstallRuntimeError {Invoke-CcodRuntimeMethod $scope.State.Runtime CopyProductShortcut @($scope.Record.Token,$Kind,[IO.Path]::GetFullPath($SourcePath),$Leaf,$ExpectedLength,$ExpectedSha256)} 'CCOD_INSTALL_PRODUCT_SHORTCUT_FAILED'|Out-Null
    [pscustomobject]@{Kind=$Kind;Leaf=$Leaf;Length=$ExpectedLength;Sha256=$ExpectedSha256}
}
