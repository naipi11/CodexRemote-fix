...
function Open-CcodInstallProductRegistrationTransaction {param([Parameter(Mandatory)][string]$InstallRoot,[Parameter(Mandatory)]$ReadyTransaction)$strictNames=@('schemaVersion','transactionId','oldRuntimeId','oldGeneration','oldManifestSha256','newRuntimeId','newGeneration','newManifestSha256','sealedPackageSha256','ownedObjectNames','phase','errorCode');if((@($ReadyTransaction.PSObject.Properties.Name)-join'|')-cne($strictNames-join'|')){Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SCOPE' 'Complete Ready transaction identity is required' $ReadyTransaction};$ReadyEvidence=[pscustomobject][ordered]@{phase=$ReadyTransaction.phase;runtimeId=$ReadyTransaction.newRuntimeId;runtimeGeneration=[uint64]$ReadyTransaction.newGeneration;manifestSha256=$ReadyTransaction.newManifestSha256;packageSha256=$ReadyTransaction.sealedPackageSha256};Initialize-CcodInstallRuntime;if(-not[IO.Path]::IsPathRooted($InstallRoot)){Throw-CcodInstallFileError 'CCOD_INSTALL_ROOT_INVALID' 'Install root must be absolute' $InstallRoot};$properties=@($ReadyEvidence.PSObject.Properties);$names=@('phase','runtimeId','runtimeGeneration','manifestSha256','packageSha256');if($properties.Count-ne$names.Count-or(@($properties.Name)-join'|')-cne($names-join'|')-or$ReadyEvidence.phase-cne'Ready'-or$ReadyEvidence.runtimeId-isnot[string]-or[uint64]$ReadyEvidence.runtimeGeneration-lt1-or$ReadyEvidence.manifestSha256-cnotmatch'^[0-9a-f]{64}$'-or$ReadyEvidence.packageSha256-cnotmatch'^[0-9a-f]{64}$'){Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SCOPE' 'Product transaction requires exact Ready evidence' $ReadyEvidence};$root=[IO.Path]::GetFullPath($InstallRoot);$pointerRoot=Join-Path $root 'state\active-generation';$pointerLeaves=@(Get-ChildItem -LiteralPath $pointerRoot -File -Force -ErrorAction Stop|Sort-Object Name);if($pointerLeaves.Count-eq0-or$pointerLeaves[-1].Name-cne('{0:D20}.json'-f[uint64]$ReadyEvidence.runtimeGeneration)){Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SCOPE' 'Ready evidence is not the latest selected generation' $ReadyEvidence};$pointerPath=Join-Path $root ('state\active-generation\{0:D20}.json'-f[uint64]$ReadyEvidence.runtimeGeneration);$manifestPath=Join-Path (Join-Path (Join-Path $root 'runtime') $ReadyEvidence.runtimeId) 'manifest.json';try{$pointer=Get-Content -LiteralPath $pointerPath -Raw -ErrorAction Stop|ConvertFrom-Json -ErrorAction Stop;if([string]$pointer.activeRuntime-cne[string]$ReadyEvidence.runtimeId-or[uint64]$pointer.generation-ne[uint64]$ReadyEvidence.runtimeGeneration-or(Get-FileHash -LiteralPath $manifestPath -Algorithm SHA256 -ErrorAction Stop).Hash.ToLowerInvariant()-cne[string]$ReadyEvidence.manifestSha256){throw 'selected pointer or manifest mismatch'};$matches=@();foreach($file in @(Get-ChildItem -LiteralPath (Join-Path $root 'state\install-transactions') -File -Force -ErrorAction Stop)){try{$record=Get-Content -LiteralPath $file.FullName -Raw -ErrorAction Stop|ConvertFrom-Json -ErrorAction Stop;$recordNames=@($record.PSObject.Properties.Name);$strictNames=@('schemaVersion','transactionId','oldRuntimeId','oldGeneration','oldManifestSha256','newRuntimeId','newGeneration','newManifestSha256','sealedPackageSha256','ownedObjectNames','phase','errorCode');if(($recordNames-join'|')-cne($strictNames-join'|')){throw 'noncanonical transaction record'};if($record.phase-ceq'Ready'-and$record.newRuntimeId-ceq$ReadyEvidence.runtimeId-and[uint64]$record.newGeneration-eq[uint64]$ReadyEvidence.runtimeGeneration-and$record.newManifestSha256-ceq$ReadyEvidence.manifestSha256-and$record.sealedPackageSha256-ceq$ReadyEvidence.packageSha256){$matches+=,$record}}catch{throw}};if($matches.Count-ne1){throw 'matching Ready record missing or ambiguous'}}catch{Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SCOPE' 'Selected pointer manifest and Ready record could not be proven' $ReadyEvidence};$arguments=[object[]]@($root,$null);$token=Convert-CcodInstallRuntimeError {Invoke-CcodRuntimeStatic OpenProduct $arguments} 'CCOD_INSTALL_PRODUCT_SCOPE';$runtime=$arguments[1];$transaction=New-CcodInstallCapability;$script:CcodTransactions.Add($transaction,[pscustomobject]@{Runtime=$runtime;InstallRoot=$root.TrimEnd('\');RuntimeId=$ReadyEvidence.runtimeId;RootToken=$token;StateOnly=$false;ProductOnly=$true;ReadyEvidence=$ReadyEvidence;ReadyCanonical=($ReadyTransaction|ConvertTo-Json -Depth 8 -Compress);Closed=$false;CleanupFailed=$false;Retired=$false;RetirementResult=$null;Disposition=$null});$script:CcodScopes.Add($transaction,[pscustomobject]@{Transaction=$transaction;Token=$token;Kind='ProductTransaction';ReadOnly=$true;RuntimeId=$ReadyEvidence.runtimeId});$transaction}
