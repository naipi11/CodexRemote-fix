...
internal sealed class CcodInstallGenerationRuntimeV7 : IDisposable
{
    ...
    private void RequireStateDirectory(Pin parent,string leaf){if(productOnly)throw new InvalidOperationException("product-only scope");if(!stateOnly&&!retryOnly)return;string error=retryOnly?"migration-retry scope":"state-only scope";if(Object.ReferenceEquals(parent,root)){if(!String.Equals(leaf,"state",StringComparison.Ordinal))throw new InvalidOperationException(error);return;}if(!IsStateScope(parent))throw new InvalidOperationException(error);}
    ...
    {
        if(stateOnly)throw new InvalidOperationException("state-only scope");if(retryOnly)throw new InvalidOperationException("migration-retry scope");if(productOnly)throw new InvalidOperationException("product-only scope");Pin parent=Require(parentToken,true);ValidateCurrent(parent);string full=Path.GetFullPath(source);
    ...
    [MethodImpl(MethodImplOptions.Synchronized)] internal void SetLegacyPlanDirectoryFailureForTest(string mode)
    {
        if(!productOnly||(mode!="Set"&&mode!="Validate"))throw new InvalidOperationException("product-only scope");legacyPlanDirectoryFailureForTest=mode;
    }
    ...
    {
        if(stateOnly)throw new InvalidOperationException("state-only scope");if(retryOnly)throw new InvalidOperationException("migration-retry scope");if(disposed)throw new ObjectDisposedException("transaction");
    ...
    {
        if(stateOnly)throw new InvalidOperationException("state-only scope");if(retryOnly)throw new InvalidOperationException("migration-retry scope");Pin parent=Require(folderToken,true);ValidateCurrent(parent);Pin source=Require(sourceToken,false);ValidateCurrent(source);
    ...
    {
        const string leaf="legacy-registration-migrations";Pin existing;if(names.TryGetValue(Key(state,leaf),out existing)){ValidateCurrent(existing);EnsureLegacyPlanDirectorySecurity(existing,true,null);return existing;}OpenResult result;
        ...
        Pin pin=null;bool registered=false;try{pin=ValidateDirectoryPin(state,leaf,Path.Combine(state.Path,leaf),result.Created,result.Handle);result.Handle=null;AddPin(pin);registered=true;string failure=legacyPlanDirectoryFailureForTest;legacyPlanDirectoryFailureForTest=null;EnsureLegacyPlanDirectorySecurity(pin,!result.Created,failure);return pin;}finally{if(!registered&&pin!=null)pin.Dispose();if(result.Handle!=null)result.Handle.Dispose();}
    }

    private Pin OpenExistingLegacyPlanDirectory(Pin state)
    {
        const string leaf="legacy-registration-migrations";Pin existing;if(names.TryGetValue(Key(state,leaf),out existing)){ValidateCurrent(existing);EnsureLegacyPlanDirectorySecurity(existing,true,null);return existing;}OpenResult result;
        try{result=OpenRelative(state.Native,leaf,LIST_DIRECTORY|READ_ATTRIBUTES|READ_CONTROL|WRITE_DAC|WRITE_OWNER|SYNC,SHARE_READ|SHARE_WRITE,OPEN,DIRECTORY|BACKUP_INTENT);}catch(Win32Exception exception){if(exception.NativeErrorCode==2||exception.NativeErrorCode==3)return null;throw;}
        Pin pin=null;bool registered=false;try{pin=ValidateDirectoryPin(state,leaf,Path.Combine(state.Path,leaf),false,result.Handle);result.Handle=null;AddPin(pin);registered=true;EnsureLegacyPlanDirectorySecurity(pin,true,null);return pin;}finally{if(!registered&&pin!=null)pin.Dispose();if(result.Handle!=null)result.Handle.Dispose();}
    }

    private void EnsureLegacyPlanDirectorySecurity(Pin pin,bool allowRecovery,string failure)
    {
        ValidateCurrent(pin);byte[] descriptor=ReadLegacyPlanSecurity(pin.Native);if(IsExactLegacyPlanSecurity(descriptor,true))return;if(allowRecovery&&(Enumerate(pin.Native).Length!=0||!IsRecoverableLegacyPlanDirectorySecurity(descriptor)))throw new InvalidDataException("legacy plan security");ValidateCurrent(pin);if(failure=="Set")throw new InvalidDataException("legacy plan test Set failure");SetExactLegacyPlanSecurity(pin.Native,true);ValidateCurrent(pin);if(failure=="Validate")throw new InvalidDataException("legacy plan test Validate failure");ValidateExactLegacyPlanSecurity(pin.Native,true);
    }
    ...
    private static bool IsExactLegacyPlanSecurity(byte[] descriptor,bool directory){return TestLegacyPlanSecurity(descriptor,directory,false);}
    internal static bool IsRecoverableLegacyPlanDirectorySecurity(byte[] descriptor){return TestLegacyPlanSecurity(descriptor,true,true);}
    private static bool TestLegacyPlanSecurity(byte[] descriptor,bool directory,bool inherited)
    {
        try{RawSecurityDescriptor raw=new RawSecurityDescriptor(descriptor,0);WindowsIdentity identity=WindowsIdentity.GetCurrent();try{SecurityIdentifier user=identity.User;bool protectedAcl=(raw.ControlFlags&ControlFlags.DiscretionaryAclProtected)!=0;if(user==null||raw.Owner==null||!raw.Owner.Equals(inherited?(identity.Owner??user):user)||protectedAcl==inherited||raw.DiscretionaryAcl==null||raw.DiscretionaryAcl.Count!=3)return false;HashSet<string> expected=new HashSet<string>(StringComparer.Ordinal){user.Value,new SecurityIdentifier(WellKnownSidType.LocalSystemSid,null).Value,new SecurityIdentifier(WellKnownSidType.BuiltinAdministratorsSid,null).Value};AceFlags expectedFlags=directory?(AceFlags.ContainerInherit|AceFlags.ObjectInherit):AceFlags.None;if(inherited)expectedFlags|=AceFlags.Inherited;foreach(GenericAce generic in raw.DiscretionaryAcl){CommonAce ace=generic as CommonAce;if(ace==null||ace.IsCallback||ace.AceQualifier!=AceQualifier.AccessAllowed||ace.AceFlags!=expectedFlags||ace.AccessMask!=(int)FileSystemRights.FullControl||ace.SecurityIdentifier==null||!expected.Remove(ace.SecurityIdentifier.Value))return false;}return expected.Count==0;}finally{identity.Dispose();}}catch{return false;}
    }
    private static void ValidateExactLegacyPlanSecurity(SafeFileHandle handle,bool directory){if(!IsExactLegacyPlanSecurity(ReadLegacyPlanSecurity(handle),directory))throw new InvalidDataException("legacy plan security");}
...
function Open-CcodInstallGeneration {
    ...
    $runtime=$arguments[2];$transaction=New-CcodInstallCapability;$script:CcodTransactions.Add($transaction,[pscustomobject]@{Runtime=$runtime;InstallRoot=[IO.Path]::GetFullPath($InstallRoot).TrimEnd('\');RuntimeId=$RuntimeId;RootToken=$token;StateOnly=$false;ProductOnly=$false;RetryOnly=$false;Closed=$false;CleanupFailed=$false;Retired=$false;RetirementResult=$null;Disposition=$null});$script:CcodScopes.Add($transaction,[pscustomobject]@{Transaction=$transaction;Token=$token;Kind='Generation';ReadOnly=$false;RuntimeId=$RuntimeId});$transaction
}
function Open-CcodInstallStateTransaction { param([Parameter(Mandatory)][string]$InstallRoot) Initialize-CcodInstallRuntime;if(-not[IO.Path]::IsPathRooted($InstallRoot)){Throw-CcodInstallFileError 'CCOD_INSTALL_ROOT_INVALID' 'Install root must be absolute' $InstallRoot};$arguments=[object[]]@([IO.Path]::GetFullPath($InstallRoot),$null);$token=Convert-CcodInstallRuntimeError {Invoke-CcodRuntimeStatic OpenState $arguments} 'CCOD_INSTALL_STATE_TRANSACTION_INVALID';$runtime=$arguments[1];$transaction=New-CcodInstallCapability;$script:CcodTransactions.Add($transaction,[pscustomobject]@{Runtime=$runtime;InstallRoot=[IO.Path]::GetFullPath($InstallRoot).TrimEnd('\');RuntimeId=$null;RootToken=$token;StateOnly=$true;ProductOnly=$false;RetryOnly=$false;Closed=$false;CleanupFailed=$false;Retired=$false;RetirementResult=$null;Disposition=$null});$script:CcodScopes.Add($transaction,[pscustomobject]@{Transaction=$transaction;Token=$token;Kind='StateTransaction';ReadOnly=$false;RuntimeId=$null});$transaction }
function Open-CcodInstallMigrationRetryTransaction { param([Parameter(Mandatory)][string]$InstallRoot) Initialize-CcodInstallRuntime;if(-not[IO.Path]::IsPathRooted($InstallRoot)){Throw-CcodInstallFileError 'CCOD_INSTALL_ROOT_INVALID' 'Install root must be absolute' $InstallRoot};$arguments=[object[]]@([IO.Path]::GetFullPath($InstallRoot),$null);$token=Convert-CcodInstallRuntimeError {Invoke-CcodRuntimeStatic OpenRetry $arguments} 'CCOD_INSTALL_MIGRATION_RETRY_SCOPE';$runtime=$arguments[1];$transaction=New-CcodInstallCapability;$script:CcodTransactions.Add($transaction,[pscustomobject]@{Runtime=$runtime;InstallRoot=[IO.Path]::GetFullPath($InstallRoot).TrimEnd('\');RuntimeId=$null;RootToken=$token;StateOnly=$false;ProductOnly=$false;RetryOnly=$true;Closed=$false;CleanupFailed=$false;Retired=$false;RetirementResult=$null;Disposition=$null});$script:CcodScopes.Add($transaction,[pscustomobject]@{Transaction=$transaction;Token=$token;Kind='MigrationRetryTransaction';ReadOnly=$false;RuntimeId=$null});$transaction }
...
function Read-CcodInstallAuthorityFile {
    ...
    try{$result=@(Invoke-CcodRuntimeStatic ReadAuthorityFile @([IO.Path]::GetFullPath($Path)));if($result.Count-ne2-or$result[0]-isnot[string]-or$result[1]-isnot[string]-or$result[1]-cnotmatch'^[0-9a-f]{64}$'){throw 'authority read result invalid'};$bytes=[Convert]::FromBase64String([string]$result[0]);return [pscustomobject]@{Bytes=$bytes;Sha256=[string]$result[1]}}
    catch{if(([string]$_.FullyQualifiedErrorId-split',')[0]-ceq'CCOD_INSTALL_PRODUCT_SCOPE'){throw};Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SCOPE' 'Authority file could not be read from one verified native handle' $Path}
}
function Get-CcodInstallLifecycleValidationModule {
    $path=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'InstallLifecycle.psm1'))
    $module=Get-Module|Where-Object{$null-ne$_.Path-and[IO.Path]::GetFullPath($_.Path)-ceq$path}|Select-Object -First 1
    if($null-eq$module){$module=Import-Module $path -PassThru -ErrorAction Stop}
    if($null-eq$module){throw 'canonical lifecycle module unavailable'}
    return $module
}
function Assert-CcodInstallCanonicalTransactionRecord($Record){
    $module=Get-CcodInstallLifecycleValidationModule
    return (&$module {param($Value)[void](Assert-CcodInstallTransactionRecord -Record $Value);Get-CcodInstallTransactionRecordLeaf -Record $Value} $Record)
}
function Resolve-CcodInstallCanonicalTransactionHead([object[]]$Records,$ActiveRuntime){
    $module=Get-CcodInstallLifecycleValidationModule
    return (&$module {param($Values,$Active)Resolve-CcodInstallTransactionRecordHead -Records $Values -ActiveRuntime $Active} $Records $ActiveRuntime)
}
function Get-CcodInstallKernelObjectsModule {
    $path=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'KernelObjects.psm1'))
    $module=Get-Module|Where-Object{$null-ne$_.Path-and[IO.Path]::GetFullPath($_.Path)-ceq$path}|Select-Object -First 1
    if($null-eq$module){$module=Import-Module $path -PassThru -ErrorAction Stop}
    if($null-eq$module){throw 'kernel coordination module unavailable'}
    return $module
}
function Enter-CcodInstallProductAuthorityLease {
    $identity=$null
    try{
        $identity=[Security.Principal.WindowsIdentity]::GetCurrent();if($null-eq$identity-or$null-eq$identity.User){throw 'current user identity unavailable'}
        $module=Get-CcodInstallKernelObjectsModule
        $lease=&$module {param($Sid)Enter-CcodMutex -Kind AccountTransition -UserSid $Sid -TimeoutMilliseconds 30000} $identity.User.Value
        if($null-eq$lease-or$lease.Outcome-cne'Acquired'){throw 'product authority coordination timed out'}
        return $lease
    }catch{Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SCOPE' 'Product authority coordination lease could not be acquired' $null}
    finally{if($null-ne$identity){$identity.Dispose()}}
}
function Exit-CcodInstallProductAuthorityLease($Lease){
    if($null-eq$Lease){return}
    $module=Get-CcodInstallKernelObjectsModule
    &$module {param($Value)Exit-CcodMutex -Lease $Value} $Lease|Out-Null
}
function Assert-CcodInstallProductAuthorityLease($State){
    $lease=if($null-ne$State.PSObject.Properties['AuthorityLease']){$State.AuthorityLease}else{$null}
    if(-not$State.ProductOnly-or$null-eq$lease-or$lease.SchemaVersion-isnot[int]-or$lease.SchemaVersion-ne1-or$lease.Kind-cne'AccountTransition'-or$lease.Outcome-cne'Acquired'-or$lease.Released-isnot[bool]-or$lease.Released-or$lease.OwnerManagedThreadId-isnot[int]-or$lease.OwnerManagedThreadId-ne[Threading.Thread]::CurrentThread.ManagedThreadId){Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SCOPE' 'Retained product authority requires its live proof-to-use coordination lease' $null}
    return $true
}
function Read-CcodInstallProductStrictJson([string]$Path,[string[]]$Fields){$read=Read-CcodInstallAuthorityFile -Path $Path;$json=[Text.UTF8Encoding]::new($false,$true).GetString($read.Bytes);foreach($field in $Fields){if((Get-CcodTopLevelJsonPropertyCount $json $field)-ne1){throw 'duplicate or missing authority member'}};$value=$json|ConvertFrom-Json -ErrorAction Stop;if(-not(Test-CcodInstallOrderedProperties $value $Fields)){throw 'noncanonical authority record'};return $value}
function Get-CcodInstallProductReadyAuthority([string]$InstallRoot,$ReadyTransaction){
    try{[void](Assert-CcodInstallCanonicalTransactionRecord $ReadyTransaction)}catch{Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SCOPE' 'Complete strict Ready transaction identity is required' $ReadyTransaction};if($ReadyTransaction.phase-cne'Ready'){Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SCOPE' 'Terminal Ready transaction is required' $ReadyTransaction}
    $root=[IO.Path]::GetFullPath($InstallRoot).TrimEnd('\');try{
        foreach($directory in @((Join-Path $root 'state'),(Join-Path $root 'state\active-generation'),(Join-Path $root 'state\install-transactions'))){$item=Get-Item -LiteralPath $directory -Force -ErrorAction Stop;if(-not$item.PSIsContainer-or($item.Attributes-band[IO.FileAttributes]::ReparsePoint)-ne0){throw 'unsafe authority directory'}}
        $pointerRoot=Join-Path $root 'state\active-generation';$pointerObjects=@(Get-ChildItem -LiteralPath $pointerRoot -Force -ErrorAction Stop|Sort-Object Name);if($pointerObjects.Count-eq0){throw 'pointer chain missing'};$latest=$null
        for($index=0;$index-lt$pointerObjects.Count;$index++){$expectedGeneration=[uint64]($index+1);$file=$pointerObjects[$index];$expectedLeaf='{0:D20}.json'-f$expectedGeneration;if($file.PSIsContainer-or$file.Name-cne$expectedLeaf){throw 'pointer chain is not contiguous'};$pointer=Read-CcodInstallProductStrictJson $file.FullName @('schemaVersion','generation','activeRuntime','previousGeneration');if($pointer.schemaVersion-isnot[int]-or$pointer.schemaVersion-ne1-or-not(Test-CcodInstallUnsignedInteger $pointer.generation)-or[uint64]$pointer.generation-ne$expectedGeneration-or$pointer.activeRuntime-isnot[string]-or$pointer.activeRuntime-cnotmatch'^[A-Za-z0-9][A-Za-z0-9._-]{0,95}$'-or-not(Test-CcodInstallUnsignedInteger $pointer.previousGeneration)-or[uint64]$pointer.previousGeneration-ne[uint64]$index){throw 'pointer record invalid'};$latest=$pointer}
        if([uint64]$latest.generation-ne[uint64]$ReadyTransaction.newGeneration-or$latest.activeRuntime-cne$ReadyTransaction.newRuntimeId){throw 'Ready transaction is stale'}
        $manifestPath=Join-Path (Join-Path (Join-Path $root 'runtime') $ReadyTransaction.newRuntimeId) 'manifest.json';$manifestRead=Read-CcodInstallAuthorityFile -Path $manifestPath;$manifestSha=$manifestRead.Sha256;if($manifestSha-cne$ReadyTransaction.newManifestSha256){throw 'manifest mismatch'}
        $store=Join-Path $root 'state\install-transactions';$records=[Collections.Generic.List[object]]::new();foreach($file in @(Get-ChildItem -LiteralPath $store -Force -ErrorAction Stop|Sort-Object Name)){if($file.PSIsContainer-or$file.Name-cnotmatch'^\d{20}\.\d{2}\.[A-Za-z]+\.[0-9a-f-]{36}\.json$'){throw 'unknown transaction object'};$record=Read-CcodInstallProductStrictJson $file.FullName $script:CcodProductReadyFields;$expectedLeaf=Assert-CcodInstallCanonicalTransactionRecord $record;if($file.Name-cne$expectedLeaf){throw 'transaction leaf is noncanonical'};$records.Add($record)}
        $canonicalHead=Resolve-CcodInstallCanonicalTransactionHead $records.ToArray() $latest;$wanted=$ReadyTransaction|ConvertTo-Json -Depth 8 -Compress;if($null-eq$canonicalHead-or$canonicalHead.phase-cne'Ready'-or($canonicalHead|ConvertTo-Json -Depth 8 -Compress)-cne$wanted){throw 'canonical lifecycle head is not the selected Ready record'}
        return [pscustomobject]@{ReadyCanonical=$wanted;PersistedReadyCanonical=($canonicalHead|ConvertTo-Json -Depth 8 -Compress);RuntimeId=[string]$ReadyTransaction.newRuntimeId;Generation=[uint64]$ReadyTransaction.newGeneration;ManifestSha256=[string]$manifestSha;PackageSha256=[string]$ReadyTransaction.sealedPackageSha256}
    }catch{if(([string]$_.FullyQualifiedErrorId-split',')[0]-ceq'CCOD_INSTALL_PRODUCT_SCOPE'){throw};Throw-CcodInstallFileError 'CCOD_INSTALL_PRODUCT_SCOPE' 'Complete selector manifest and persisted Ready authority could not be proven' $ReadyTransaction}
}
...
function Open-CcodInstallProductRegistrationTransaction {
    ...
    Initialize-CcodInstallRuntime;if(-not[IO.Path]::IsPathRooted($InstallRoot)){Throw-CcodInstallFileError 'CCOD_INSTALL_ROOT_INVALID' 'Install root must be absolute' $InstallRoot};$root=[IO.Path]::GetFullPath($InstallRoot);$lease=$null
    try{
        $lease=Enter-CcodInstallProductAuthorityLease
        $authority=Get-CcodInstallProductReadyAuthority $root $ReadyTransaction
        $arguments=[object[]]@($root,$null);$token=Convert-CcodInstallRuntimeError {Invoke-CcodRuntimeStatic OpenProduct $arguments} 'CCOD_INSTALL_PRODUCT_SCOPE';$runtime=$arguments[1];$transaction=New-CcodInstallCapability;$script:CcodTransactions.Add($transaction,[pscustomobject]@{Runtime=$runtime;InstallRoot=$root.TrimEnd('\');RuntimeId=$authority.RuntimeId;RootToken=$token;StateOnly=$false;ProductOnly=$true;RetryOnly=$false;ReadyAuthority=$authority;AuthorityLease=$lease;Closed=$false;CleanupFailed=$false;Retired=$false;RetirementResult=$null;Disposition=$null});$script:CcodScopes.Add($transaction,[pscustomobject]@{Transaction=$transaction;Token=$token;Kind='ProductTransaction';ReadOnly=$true;RuntimeId=$authority.RuntimeId});$lease=$null;return $transaction
