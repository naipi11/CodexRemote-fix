...
$requiredTransactionSelfTest = 'InstallFileTransaction.SelfTest.ps1'
if (@($tests | Where-Object Name -CEQ $requiredTransactionSelfTest).Count -ne 1) {
    throw "Required install file transaction self-test is not registered exactly once: $requiredTransactionSelfTest"
}
