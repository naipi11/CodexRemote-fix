...
Invoke-CcodTest '2.5.20 source metadata binds the package and native assembly identities' {
    $package=Get-Content -LiteralPath (Join-Path $repositoryRoot 'package.json') -Raw|ConvertFrom-Json
    Assert-CcodEqual '2.5.20' ([string]$package.version) 'package version is the 2.5.20 release version'
    $assemblyInfo=Get-Content -LiteralPath (Join-Path $repositoryRoot 'src\trayhost\AssemblyInfo.cs') -Raw
    Assert-CcodTrue ($assemblyInfo -match 'AssemblyVersion\("2\.5\.20\.0"\)') 'TrayHost assembly version is 2.5.20.0'
    Assert-CcodTrue ($assemblyInfo -match 'AssemblyFileVersion\("2\.5\.20\.0"\)') 'TrayHost file version is 2.5.20.0'
    $manifest=Get-Content -LiteralPath (Join-Path $repositoryRoot 'src\trayhost\CodexRemote.TrayHost.manifest') -Raw
    Assert-CcodTrue ($manifest -match '<assemblyIdentity version="2\.5\.20\.0"') 'TrayHost manifest identity is 2.5.20.0'
    $portableManifest=Get-Content -LiteralPath (Join-Path $repositoryRoot 'src\portable\CodexRemote.Portable.manifest') -Raw
    Assert-CcodTrue ($portableManifest -match '<assemblyIdentity version="2\.5\.20\.0"') 'portable launcher manifest identity is 2.5.20.0'
    $portableAssemblyInfo=Get-Content -LiteralPath (Join-Path $repositoryRoot 'src\portable\AssemblyInfo.cs') -Raw
    Assert-CcodTrue ($portableAssemblyInfo -match 'AssemblyVersion\("2\.5\.20\.0"\)') 'portable launcher assembly version is 2.5.20.0'
    Assert-CcodTrue ($portableAssemblyInfo -match 'AssemblyFileVersion\("2\.5\.20\.0"\)') 'portable launcher file version is 2.5.20.0'
}
...
Invoke-CcodTest 'TrayHost build emits one source-auditable artifact and rejects tampering' {
    ...
    try{
        $result=Invoke-CcodTrayHostBuild -RepositoryRoot $repositoryRoot -Version '2.5.20' -OutputDirectory $artifact
        ...
        Assert-CcodEqual '2.5.20' ([string]$provenance.version) 'provenance version is exact'
        $fileVersion=[Diagnostics.FileVersionInfo]::GetVersionInfo((Join-Path $artifact 'CodexRemote.TrayHost.exe')).FileVersion
        Assert-CcodEqual '2.5.20.0' ([string]$fileVersion) 'built TrayHost file version is release-aligned'
        ...
        $validated=Test-CcodTrayHostArtifact -RepositoryRoot $repositoryRoot -Version '2.5.20' -ArtifactDirectory $artifact -ExpectedGitCommit $currentCommit
        ...
        $threw=$false; try{Test-CcodTrayHostArtifact -RepositoryRoot $repositoryRoot -Version '2.5.20' -ArtifactDirectory $artifact|Out-Null}catch{$threw=$true}
...
Invoke-CcodTest 'portable launcher build emits a tamper-bound double-click entrypoint' {
    ...
    try {
        $result = Invoke-CcodPortableLauncherBuild -RepositoryRoot $repositoryRoot -Version '2.5.20' -OutputDirectory $artifact
        ...
        Assert-CcodEqual '2.5.20' ([string]$provenance.version) 'portable launcher provenance version is exact'
        $fileVersion = [Diagnostics.FileVersionInfo]::GetVersionInfo((Join-Path $artifact 'CodexRemote.Portable.exe'))
        Assert-CcodEqual '2.5.20.0' ([string]$fileVersion.FileVersion) 'portable launcher Windows file version is release-aligned'
        Assert-CcodEqual '2.5.20.0' ([string]$fileVersion.ProductVersion) 'portable launcher Windows product version is release-aligned'
