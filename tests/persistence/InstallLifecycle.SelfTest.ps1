...
$results += Invoke-CcodTest 'portable builder publishes the exact CodexRemote-fix 2.5.12 release artifact contract' {
    $package = Get-Content -LiteralPath (Join-Path $repositoryRoot 'package.json') -Raw | ConvertFrom-Json
    Assert-CcodEqual '2.5.12' ([string]$package.version) 'package version is exactly 2.5.12'
...
$results += Invoke-CcodTest 'README and release workflow publish current portable-release branding' {
    ...
    Assert-CcodTrue ($quickStart -cmatch 'CodexRemote-fix-2\.5\.12-setup\.exe') 'English Quick Start names the setup installer'
    Assert-CcodTrue ($quickStart -cmatch 'CodexRemote-fix-2\.5\.12-windows-x64\.zip') 'English Quick Start names the exact portable artifact'
    ...
    Assert-CcodTrue ($quickStartChinese -cmatch 'CodexRemote-fix-2\.5\.12-setup\.exe') 'Chinese Quick Start names the setup installer'
    Assert-CcodTrue ($quickStartChinese -cmatch 'CodexRemote-fix-2\.5\.12-windows-x64\.zip') 'Chinese Quick Start names the exact portable artifact'
