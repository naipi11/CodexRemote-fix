...
Invoke-CcodTest 'product registration transaction exposes only a selected retained manifest file capability' {
    ...
    try{
        ...
        $ready=[pscustomobject][ordered]@{phase='Ready';runtimeId=$runtimeId;runtimeGeneration=[uint64]1;manifestSha256=$manifest.Sha256;packageSha256=('d'*64)};$transactions=Join-Path $fixture.Install 'state\install-transactions';[IO.Directory]::CreateDirectory($transactions)|Out-Null;[IO.File]::WriteAllText((Join-Path $transactions 'ready.json'),([ordered]@{phase='Ready';newRuntimeId=$runtimeId;newGeneration=[uint64]1;newManifestSha256=$manifest.Sha256;sealedPackageSha256=('d'*64)}|ConvertTo-Json -Compress),[Text.UTF8Encoding]::new($false));$product=Open-CcodInstallProductRegistrationTransaction -InstallRoot $fixture.Install -ReadyEvidence $ready;$fixture.Transactions.Add($product);$retained=Open-CcodInstallRetainedGeneration -InstallRoot $fixture.Install -RuntimeId $runtimeId -ExpectedManifestSha256 $manifest.Sha256 -FileTransaction $product;$file=Open-CcodInstallRetainedFile -Generation $retained -RelativePath 'registration/StartMenu.CodexRemote-fix.lnk' -ReadyEvidence $ready
        Assert-CcodTrue ($null-ne$file) 'selected manifest file returns an opaque retained source capability'
        Assert-CcodThrows {Open-CcodInstallRetainedFile -Generation $retained -RelativePath 'C:\outside\arbitrary.lnk' -ReadyEvidence $ready|Out-Null} 'CCOD_INSTALL_PRODUCT_SHORTCUT_INVALID'
        ...
        $preReady=$ready.PSObject.Copy();$preReady.phase='ProtectionReady';Assert-CcodThrows {Open-CcodInstallProductRegistrationTransaction -InstallRoot $fixture.Install -ReadyEvidence $preReady|Out-Null} 'CCOD_INSTALL_PRODUCT_SCOPE'
        [IO.File]::WriteAllText((Join-Path $fixture.Install 'state\active-generation\00000000000000000002.json'),'{'+'"schemaVersion":1,"generation":2,"activeRuntime":"sibling","previousGeneration":1}',[Text.UTF8Encoding]::new($false));Assert-CcodThrows {Open-CcodInstallProductRegistrationTransaction -InstallRoot $fixture.Install -ReadyEvidence $ready|Out-Null} 'CCOD_INSTALL_PRODUCT_SCOPE'
