...
internal static class TrayHostNativeSelfTest
{
    ...
    private static PresentationSnapshot SnapshotV2(ulong revision)
    {
        string[] strings = new string[] {
            "CodexRemote-fix 2.5.6", "Connection: Connected", "Protection: Running", "Check and repair remote connection",
            "Language / 语言", "Follow system (English)", "中文", "English", "Open logs", "About", "Exit",
            "About", "CodexRemote-fix | Version 2.5.6", "Exit CodexRemote-fix?", "Remote control will stop.", "Action failed"
        };
    ...
    private static void TestSimplifiedMenuAndExitConfirmation()
    {
        ...
        AssertEqual("CodexRemote-fix 2.5.6|Connection: Connected|Protection: Running|Check and repair remote connection|Language / 语言|Open logs|About|Exit", String.Join("|", platform.AppendedText.ToArray()), "menu contains only the approved information architecture");
    ...
    private static void TestVerifiedAboutUsesTheAcknowledgedSnapshotVersion()
    {
        ...
        AssertEqual("About|CodexRemote-fix | Version 2.5.6", platform.MessageBoxes[0], "verified About displays the version from the acknowledged snapshot");
        window.Dispose();
    }
