...
Task 2: initial implementation `148f6ee`, evidence `66a91aa`; scoped review FAIL (0 Critical, 3 Important). Fix round 1 is in progress. Required closure: crash-safe fixed-directory ACL initialization and registered-handle cleanup; exact nested profile/raw-path schema; real child-process replay plus published-write/read-back failure retry evidence.

The prior frozen gate passed but is invalidated by the required Task 2 source changes. Successor final gate and fresh scoped review: pending. Tasks 5–8 remain blocked.
